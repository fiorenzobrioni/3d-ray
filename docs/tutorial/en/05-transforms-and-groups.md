...
`imports:` section lets you split a scene across several files — for
example a file of object templates, or a font template generated by
`FontGen` under `scenes/assets/fonts/`.

```yaml
imports:
  - path: "templates/props.yaml"            # your own template file
  - path: "assets/fonts/font-open-sans.yaml" # FontGen-generated character templates
```

> Ready-made materials and lights are **not** imported — they live in the
> copy-paste preset catalogues under `scenes/presets/`. You copy the block
> you want straight into your scene's `materials:` / `lights:` section
> (see Chapter 10). `imports:` is for genuinely splitting your own scene
> into multiple files and for pulling in generated template files.

### How Imports Work

1. Paths are resolved **relative to the importing file's directory**.
   If your scene is at `scenes/my-scene.yaml`, the path
   `"assets/fonts/font-open-sans.yaml"` resolves to
   `scenes/assets/fonts/font-open-sans.yaml`.
...
3. **Local definitions win.** If both the imported file and your scene
   define a material or template with the same `id`, your local version
   takes precedence — so you can pull in a template file and then
...
### Example: Overriding an Imported Template's Material

```yaml
imports:
  - path: "assets/fonts/font-open-sans.yaml"   # Uses "font_material"

materials:
  # Define the material that all imported font characters reference
  - id: "font_material"
    type: "disney"
    color: [0.95, 0.7, 0.2]
    metallic: 1.0
...
- Copy ready-made materials from the preset catalogues in
  `scenes/presets/` instead of hand-authoring every surface.
- Use templates for repeated objects.

**Large scenes (200+ entities):**
- Split into multiple files via `imports:`: one for object templates,
  one for the main scene layout.
- Build your palette from the preset catalogues (Chapter 10).
