...
**Strada 2 — MNEE ✅** Implementata (vedi DEVLOG §Ciclo Caustiche Fase 2). Walker Newton-Raphson sulla manifold speculare (`Rendering/ManifoldWalker.cs`): trova `x → p₁(→ p₂) → luce` che soddisfa Snell/riflessione. Residuo unificato (half-vector generalizzato `η_a·ω_a + η_b·ω_b`, componente tangenziale = 0) per riflessione + rifrazione a 1/2 interfacce; Jacobiano per differenze finite in `(u,v)` (niente ray cast nel loop); termine geometrico `G = dΩ_x/dA_y` per perturbazione della luce e ri-solve. La soppressione dell'emissione sul cammino forward (stato "caustic carrier" in `TraceRay`) evita il doppio conteggio del fuoco sul path BSDF-sampled. **Nota (vedi DEVLOG §Ciclo Caustiche — niente più "base scura")**: lo shadow ray dritto NON è più reso opaco ai caster (`BlockCausticCasters` rimosso) — lo shadow trasparente passa sempre e MNEE aggiunge il fuoco sopra, così un receiver non va al buio dove MNEE non ricostruisce il trasporto; il prezzo è un lieve double-count dell'energia media trasmessa nella regione focale.
...
**Strada 2c bis — CSG world/transform + copertura group ✅** Fix dei buchi di copertura del caster CSG e dei group (scena diagnostica `scenes/showcases/glass-caustics.yaml`). Tre bug:
1. **CSG con leaf trasformato** (il caso comune: un solido booleano costruito da primitive con `translate`/`rotate` propri). La chart veniva recuperata via `rec.HitPrimitive`, che è la primitiva **object-space** dentro il `Transform` del leaf → `EvaluateManifold` risolveva attorno all'origine invece che alla posizione reale. Fix: `Transform.Hit` ora rimappa `HitPrimitive` a **se stesso** (la `IManifoldSurface` world-space corrispondente) quando non-null, preservando il `null` delle regioni piatte (cap di cilindro/cono) così il seeder le salta ancora. Il percorso analitico (`AnalyticManifoldCaster`) usa già `_surface` direttamente: invariato bit-per-bit.
2. **CSG sotto transform d'entità** (`Transform(CsgObject)`, da un `translate:` sull'entità `type: csg`). Prima rigettato da gate e builder. Fix: `Transform.CreateManifoldCaster` riconosce un `IManifoldCaster` interno e lo avvolge in `TransformedManifoldCaster` (semina in object space mappando gli endpoint con la matrice inversa, poi solleva ogni chart in world via `TransformedChart`; il clamp di membership resta in object space rimappando il vertice convergente); `CanCastCaustics` accetta `Transform(CsgObject)` con frontiera curva.
3. **Caster dentro `type: group`** (incluso group annidati e CSG nei group). Prima i flag `caustic_caster`/`caustic_receiver` erano onorati solo nel loop d'entità top-level, mai in `BuildChildList` → i figli flaggati nei group non si registravano. Fix: helper condiviso `SceneLoader.ApplyCausticFlags` chiamato sia al top-level sia per ogni figlio di group, con la matrice **group→world** accumulata (composta lungo la catena di group annidati) per registrare la geometria di seeding in world space; un group flaggato in toto emette warning (non è un singolo caster). Instance/template esclusi (geometria condivisa fra istanze, warning). Test: `CsgCausticTests` (leaf trasformato + CSG sotto transform), `GroupCausticTests`.
...
**Decisione corrente**: Strada 1 (default) + Strada 2 / 2b / 2c (+ 2c bis) ✅ + Strada 2d tier 1 ✅ (un solo opt-in: `--caustics on`; le entità sono **auto-classificate**, vedi sotto) coprono le caustiche di lente su tutta la geometria curva (primitive, mesh smooth, CSG) **in tutti gli spazi** — world, sotto transform d'entità, e dentro `type: group` annidati. Le mesh castano nel caso comune (vertice ≤1 faccia dal seed). Strada 2d tier 2 (edge-walk in-solve + planar-mirror NEE) e Strada 3 (multi-bounce/dispersive) restano roadmap.

**Auto-classificazione caster/receiver** ✅ (vedi DEVLOG §Ciclo Caustiche — auto-classificazione): con `--caustics on` non servono più flag YAML. Receiver è il default di ogni superficie (incluso `world.ground`); caster è automatico se la **geometria** focalizza (`CanCastCaustics`) **e** il **materiale** è speculare/trasmissivo (nuovo gate `MaterialCanCast` via `IMaterial.GetCausticInterface().IsCaster`). Un caster non è auto-receiver (`receiver = !caster`). I flag `caustic_caster`/`caustic_receiver` restano come **override opzionali a 3 stati** (`bool?`: assente=auto, `true`=forza, `false`=escludi) — scalpello di performance/regia per scene con molti caster. Costo zero off-path (`_enableCaustics`) e bit-identico con `--caustics off`.

**Copertura luci caustiche** ✅ (vedi DEVLOG §Ciclo Caustiche — emettitori virtuali finiti): `area`/geometriche, `sphere` (campionamento d'area esatto), e `point`/`spot` (modellate come **bulbo sferico finito** di raggio `soft_radius`, default `0.05`) innescano MNEE/SMS. La sfera implementa `TrySampleEmissivePoint` (uniforme su `4πR²`, `L_e = Color·Intensity`); point/spot campionano il bulbo (`L_e = Color·Intensity/(πr²)`, `pdf_A = 1/(4πr²)`, contributo invariante in `r`), con `SpotLight.DirectionalEmissionScale` per la falloff di cono. Rimosso il gate `if (light.IsDelta) continue;` in `ComputeCaustics`: ora a filtrare è solo `TrySampleEmissivePoint` (false ⇒ skip a costo zero). CLI `--mnee-samples N` per ripulire il rumore del bulbo finito.

**Limiti noti delle caustiche** (per quando si riprende la fase): (1) **luci direzionali/ambiente**: `directional`/sole (anche con `angular_radius > 0`) ed environment/HDRI non innescano MNEE/SMS — l'estimatore campiona un punto sull'**area** di un emettitore *finito* (`pdf_A`, perturbazione di `y` per `G`), che una sorgente all'infinito non possiede; servirebbe una connessione direzionale (target = direzione). (2) **"Vetro dentro vetro" — IOR relativo ✅ (path forward + MNEE single-caster)** (vedi DEVLOG §Ciclo IOR relativo): la rifrazione forward (`Dielectric`/`DisneyBsdf`) e l'`EtaOnSide` del `ManifoldWalker` ora usano l'IOR **relativo** `η_in/η_out` del medium in cui il raggio si trova davvero, non più sempre l'aria. Nuovo `IorStack` per-raggio (Fase A), `HitRecord.RelativeEta`, `CausticInterface.AmbientIor` (Fase B) e risoluzione geometrica del medium che racchiude un caster annidato in `TryGetCausticInterface` (Fase C, `ResolveAmbientIor`). Nel path forward (camera) il vino rifrange ora contro il cristallo (1.70) sui fianchi e contro l'aria sul pelo libero — differenza visibile su `cristallo.yaml` con caustiche off. **Residuo (caustica del vino racchiuso)**: la risoluzione ambiente di Fase C è corretta ma **inerte** sul vino completamente racchiuso, perché la connessione MNEE isolata al vino attraversa il cristallo che lo avvolge e `SegmentOccluded` la scarta sempre (il fuoco rosso sul tavolo viene dal path forward, non da MNEE attraverso il vino). Serve il vero cammino a 4 interfacce su due dielettrici annidati distinti come **singolo** solve Newton multi-chart (aria→cristallo→vino→cristallo→aria): `MaxVertices`/`MaxMneeInterfaces = 4`, IOR/assorbimento per-segmento, seeding ordinato su chart diversi e visibilità che attraversi i caster intermedi, con rischio convergenza/firefly elevato. La Fase C aiuta invece i caster annidati **parzialmente esposti**.
