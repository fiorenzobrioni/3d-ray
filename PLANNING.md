...
**Decisione corrente**: Strada 1 (default) + Strada 2 / 2b / 2c ✅ (opt-in via `--caustics on` + flag per-entità) coprono le caustiche di lente su tutta la geometria curva (primitive, mesh smooth, CSG). Strada 2d (mesh edge-crossing + planar-mirror NEE) e Strada 3 (multi-bounce/dispersive) restano roadmap.
