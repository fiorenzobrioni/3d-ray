...
- 🔬 **Caustiche focalizzate (MNEE + SMS)** — macchie di luce concentrate prodotte da rifrazione e riflessione speculare: fuochi di lente, chiazze luminose sul fondo di una ciotola di cristallo, iridescenza da prisma, riflessi da metallo spazzolato. Funziona su tutta la geometria curva — sfere, cilindri, toro, mesh smooth, solidi CSG — illuminata da luci d'area/geometriche, sfere, e sorgenti `point`/`spot` (modellate come piccolo bulbo sferico finito). Si attivano con il solo `--caustics on` (di default sui preset `final`/`ultra`): il motore **auto-classifica** ogni entità in caster (geometria curva + materiale speculare/trasmissivo) o ricevente (tutto il resto, incluso il `ground`). I flag YAML `caustic_caster` / `caustic_receiver` restano come **override opzionali a 3 stati** (assente = auto, `true` = forza, `false` = escludi). Vedi [Path Tracing e Illuminazione §2.5](./docs/technical/path-tracing-and-lighting.md).
...
| `--caustics` | — | `off`¹ | Caustiche focalizzate: `on` attiva il solver e **auto-classifica** ogni entità — diventa **caster** se ha geometria curva (primitiva sfera/cilindro/cono/capsula/toro, mesh smooth o solido CSG con frontiera curva) **e** materiale speculare/trasmissivo (`dielectric`, `metal`, o Disney con `spec_trans ≥ 0.5` non thin-walled / `metallic ≈ 1`); ogni altra superficie (incluso il `ground`) è **ricevente**. Vetro/specchio **liscio** → Manifold Next Event Estimation; vetro **frosted/rough** (`roughness > 0.04`) e metallo spazzolato → Specular Manifold Sampling. I flag YAML `caustic_caster`/`caustic_receiver` restano come override opzionali a 3 stati (assente = auto, `true` = forza, `false` = escludi). Con `off` il rendering è identico a prima e il costo è nullo. Catene fino a 2 interfacce (vetro solido). Sorgenti: luci d'area/geometriche, `sphere`, e `point`/`spot` (queste ultime modellate come piccolo bulbo sferico finito di raggio `soft_radius`, default `0.05`; più piccolo = più netto e rumoroso). `directional`/sole e cielo/HDRI non castano. ¹Attivo di default sui preset `final`/`ultra` (un `--caustics` esplicito ha la precedenza). |
| `--sms-samples` | — | `4`² | Numero di prove stocastiche di Specular Manifold Sampling per connessione su un caster rough/frosted. Valori più alti = caustiche frosted più morbide e meno rumorose, render più lento. Nessun effetto sui caster lisci (MNEE) o con `--caustics off`. ²`8` sui preset `final`/`ultra`. |
| `--mnee-samples` | — | `1` | Campioni dell'emettitore per ricevente per luce nel solver MNEE. Alzalo per ripulire le caustiche di `point`/`spot` (il bulbo virtuale finito è più rumoroso di una luce d'area). Nessun effetto con `--caustics off`. |
