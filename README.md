...
| `--caustics` | — | `off`¹ | Caustiche focalizzate: `on` attiva il solver che concentra la luce attraverso le entità marcate `caustic_caster` sulle superfici `caustic_receiver`. Vetro/specchio **liscio** → Manifold Next Event Estimation; vetro **frosted/rough** (`roughness > 0.04`) e metallo spazzolato → Specular Manifold Sampling. Opt-in per-entità nello YAML: senza i flag (o con `off`) il rendering è identico a prima e il costo è nullo. Caster su tutta la geometria curva — primitive (sfera, cilindro, cono, capsula, toro), mesh smooth e solidi CSG con frontiera curva — catene fino a 2 interfacce (vetro solido). Sorgenti: luci d'area/geometriche, `sphere`, e `point`/`spot` (queste ultime modellate come piccolo bulbo sferico finito di raggio `soft_radius`, default `0.05`; più piccolo = più netto e rumoroso). `directional`/sole e cielo/HDRI non castano. ¹Attivo di default sui preset `final`/`ultra` (un `--caustics` esplicito ha la precedenza). |
