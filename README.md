...
- 🔬 **Caustiche focalizzate (MNEE + SMS)** — Manifold Next Event Estimation concentra la luce attraverso vetro/cristallo **liscio** e specchi (`caustic_caster`) sulle superfici riceventi (`caustic_receiver`), per le vere caustiche di lente che gli shadow ray dritti non producono; **Specular Manifold Sampling** estende il solver al vetro **frosted/rough** e al metallo spazzolato con una perturbazione stocastica della manifold (`--sms-samples`). Opt-in via `--caustics on` (default sui preset `final`/`ultra`). Vedi [Path Tracing e Illuminazione §2.5](./docs/technical/path-tracing-and-lighting.md).
