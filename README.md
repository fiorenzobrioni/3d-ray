...
- 🏔 **Marble** — marmo procedurale production-grade allineato a Arnold/Cycles/RenderMan/Mitsuba. Campo ridged multifractale multi-scala (1-3 layer compositati via soft-max), recursive (IQ) domain warp che uccide ogni tiling visibile, fold geologico anisotropo, variazione cromatica di fondo e impurità minerali (Voronoi inline o `impurities_texture` esterna). Knob: `warp_amplitude/iterations`, `fold_amplitude`, `vein_layers/scale/weight`, `vein_thickness/softness`, `color_variation`, `impurities_*`. Defaults pro per Carrara out-of-the-box; recipe documentate per Calacatta, Statuario, Arabescato, Verde Alpi, Nero Marquinia.
- 🪵 **Wood** — legno procedurale production-grade allineato a Arnold/Cycles/RenderMan/Mitsuba. Profilo anulare ASIMMETRICO earlywood/latewood (sostituisce il legacy `pow(triangle, sharpness)` simmetrico), variazione random per anello (`ring_color_variation` + `ring_width_variation` — IL feature che separa "wood CG" da "wood reale"), recursive (IQ) domain warp + fold geologico anisotropo, multi-banda noise (grain + figure con `figure_aspect` per allineamento perpendicolare alla venatura), vasi open-pore via Worley anisotropico assialmente (`pore_density`/`pore_aspect`/`pore_scale` — quercia, frassino, noce, mogano), gradiente sapwood/heartwood (`heartwood_radius`/`heartwood_blend`), nodi a proiezione cono 3D con cuore scuro, `space_stretch` per tagli non isotropi, e `output: "mask"` per pilotare `roughness_texture`/`sheen_texture` Disney dal pattern degli anelli. Defaults pro per quercia out-of-the-box; recipe documentate per curly maple, flame mahogany, quartersawn oak, knotty pine, bird's-eye, burr walnut, shou-sugi-ban.
