...
- 🔬 **Caustiche focalizzate (photon mapping)** — macchie di luce concentrate prodotte da rifrazione e riflessione speculare: fuochi di lente, chiazze luminose sul fondo di una ciotola di cristallo, iridescenza da prisma, riflessi da metallo spazzolato. Un **pre-pass di photon mapping** emette fotoni di caustica dalle luci, li traccia attraverso le superfici speculari (specchio/vetro/acqua/metallo) e li deposita dove atterrano su superfici diffuse (cammini `L S+ D`); la passata camera li raccoglie con una stima di densità ai k-nearest. È **generale**: funziona su qualunque geometria speculare e con **tutti** i tipi di luce, comprese quelle **direzionali/sole** (cosa che il vecchio solver manifold non poteva fare). **Nessun flag YAML per-oggetto.** Si attivano con il solo `--caustics on` (di default sui preset `final`/`ultra`); `--caustic-photons <N>` regola il budget di fotoni. Vedi [Path Tracing e Illuminazione §2.5](./docs/technical/path-tracing-and-lighting.md).
