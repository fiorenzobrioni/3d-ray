...
- 🔬 **Caustiche focalizzate (MNEE)** — Manifold Next Event Estimation: un solver di Newton-Raphson sulla manifold speculare concentra la luce attraverso vetro/cristallo (`caustic_caster`) sulle superfici riceventi (`caustic_receiver`), per le vere caustiche di lente che gli shadow ray dritti non producono. Opt-in via `--caustics on`; unbiased e a conteggio singolo. Vedi [Path Tracing e Illuminazione §2.5](./docs/technical/path-tracing-and-lighting.md).
...
| `--caustics` | — | `off` | Caustiche focalizzate via **Manifold Next Event Estimation**: `on` attiva il walk di Newton-Raphson che concentra la luce attraverso le entità marcate `caustic_caster` (vetro/cristallo liscio o specchio) sulle superfici `caustic_receiver`. Opt-in per-entità nello YAML: senza i flag (o con `off`) il rendering è identico a prima e il costo è nullo. Fase 2: caster sferici, catene fino a 2 interfacce (vetro solido), luci d'area. |
